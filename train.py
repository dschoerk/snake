...
for i_episode in range(num_episodes):
    ...
    for t in count():
        ...
        if i_episode % 10 == 0 and i_episode > 0:
            print(f"Episode {i_episode}, Step {t}, Loss: {loss.item()}, Best Length: {best_len}")

            # run 100 simulations to evaluate the model
            total_reward = 0
            for _ in range(100):
                game = SnakeGame(wnd=wnd, field_size=(20, 20))
                game.reset(random_field_size=True)
                ...
                while True:
                    ...
                    observation, reward, terminated = game.update(action.item())
                    if terminated:
                        total_reward += game.gamestate.reward
                        break
            print(f"Average reward: {total_reward / 100}")
        if done:
            # episode_durations.append(t + 1)
            # plot_durations()
            # print(observation.reshape(10, 10))

            # print(game.gamestate.reward, )
